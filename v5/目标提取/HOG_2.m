function hogImg = HOG_2(img)
    ...
    Grad = sqrt(Gx.^2 + Gy.^2); 
    Phase = atan2d(Gy, Gx); % atan2d 自动处理分母为零的情况
        ...
        for j = 1:numCellsY
            ...
            for x = 1:(xEnd - xStart + 1)
                for y = 1:(yEnd - yStart + 1)
                    ang = angTmp(y, x);
                    binIdx = mod(floor(ang / angle), K) + 1; 
            ...
            for k = 1:K
                ...
                y1 = yCenter - dy;
                y2 = yCenter + dy;
                hogImg = insertShape(hogImg, 'Line', [x1, y1, x2, y2], 'Color', 'white', 'LineWidth', 1);
            end
        end
    end
